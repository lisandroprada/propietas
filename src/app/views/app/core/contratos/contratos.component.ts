...
export class ContratosComponent implements OnInit {
  ...
  @ViewChild('addNewModalRef', { static: true }) addNewModalRef: ContratoModalComponent;
  ...
  loadData(pageSize: number = 3, currentPage: number = 1, search: string = '', orderBy: string = '') {
    ...
    this.contratoService.getContratos(pageSize, currentPage, search, orderBy).subscribe(
      (data: any) => {
        if (data.status) {
  ...
  onContextMenuClick(action: string, item: Contrato) {
    console.log('onContextMenuClick -> action :  ', action, ', item.identityCard :', item._id);
  }
