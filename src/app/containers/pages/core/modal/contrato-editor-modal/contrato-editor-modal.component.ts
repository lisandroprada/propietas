...
export class ContratoEditorModalComponent implements OnInit {
  ...
  cargarFormulario() {
    ...
    const moneda = { style: 'currency', currency: 'ARS', minimumFractionDigits: 2 };
    ...
    cambio = isLocador2 ? this._cadena.stringInject(this.contratoModelo, {locadorAdicional: this.diccionario.locadorAdicional }) : this._cadena.stringInject(this.contratoModelo, {locadorAdicional: ' '});
    this.contratoModelo = cambio;
    cambio = isLocatario2 ? this._cadena.stringInject(this.contratoModelo, {locatarioAdicional: this.diccionario.locatarioAdicional }) : this._cadena.stringInject(this.contratoModelo, {locatarioAdicional: ' '});
    this.contratoModelo = cambio;
    cambio = isFiador2 ? this._cadena.stringInject(this.contratoModelo, {fiadorAdicional: this.diccionario.fiadorAdicional }) : this._cadena.stringInject(this.contratoModelo, {fiadorAdicional: ' '});
    this.contratoModelo = cambio;
    ...
    const data =  {
      ...
      locatarioDireccion2: isLocatario2 ? datos.locatario_data2.addressLine1 : '',
      ...
      montoAlquilerNumero: Number(datos.data.amount_month).toLocaleString('es-AR', moneda ),

      depositoGarantiaLetra: this._numLetra.numeroALetras(datos.data.amount_month).toUpperCase(),
      depositoGarantiaNumero: Number(datos.data.amount_month).toLocaleString('es-AR', moneda ),

      montoTotalLetras: this._numLetra.numeroALetras(montoTotal).toUpperCase(),
      montoAlquilerLetras: this._numLetra.numeroALetras(datos.data.amount_month).toUpperCase(),

      fechaAlta: fechaAlta.locale('es').format('dddd, D [de] MMMM [de] YYYY'),
      fechaComienzo: fechaComienzo.locale('es').format('dddd, D [de] MMMM [de] YYYY'),
      mesInicioLetra: this.capitalize(fechaComienzo.locale('es').format('dddd, MMMM Do YYYY')),

      fechaBase: fechaComienzo.locale('es').format('dddd, D [de] MMMM [de] YYYY'),
      fechaUno: fechaComienzo.add(1, 'year').locale('es').format('dddd, D [de] MMMM [de] YYYY'),
      fechaDos: fechaComienzo.add(1, 'years').locale('es').format('dddd, D [de] MMMM [de] YYYY'),

      fechaFinalizacion: fechaFinalizacion.subtract(1, 'days').add(3, 'years').locale('es').format('dddd, D [de] MMMM [de] YYYY'),
      diaMesInicio: fechaComienzo.format('D'),
      diaInicioNumero: fechaComienzo.format('D'),
      diaGraciaNumero: fechaComienzo.add(datos.data.payment_term, 'days').format('D'),
      ...
      elLocador2: isLocador2 ? this.buscaPalabra('el', datos.locador_genero2 ) : '',
      elLocatario: this.buscaPalabra('el', datos.locatario_genero ),
      elLocatario2: isLocatario2 ? this.buscaPalabra('el', datos.locatario_genero2 ) : '',
      ...
      tituloLocador2: isLocador2 ? this.buscaPalabra('señor',   datos.locador_genero2 ) :  '',
      tituloLocatario2: isLocatario2 ? this.buscaPalabra('señor', datos.locatario_genero2) : '',
      tituloFiador2: isFiador2 ? this.buscaPalabra('señor',  datos.fiador_genero2 ) : '',
      ...
      responderáLocatario: this.buscaVerbo('responderá', datos.locatario_genero),
      recibeLocatario: this.buscaVerbo('recibe', datos.locatario_genero),
      comprometeLocatario: this.buscaVerbo('compromete', datos.locatario_genero),
      ...
      notificareLocatario: this.buscaVerbo('notificare', datos.locatario_genero),
  ...
  cargarContratoModelo() {
    let i = 0;
    let contratoTexto = '';
    this._contratoService.getContratoModelo()
      .subscribe( (data: any) => {
